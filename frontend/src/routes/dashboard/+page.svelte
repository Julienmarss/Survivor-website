<script>
    ...
    async function loadDashboardData() {
        try {
            ...
            if (user && !isAdmin(user)) {
                const userProjectsResponse = await fetch(`${API_BASE}/users/${user.id}/projects`);
    ...
    function getActivityIcon(type) {
        const icons = {
            ...
            investment: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1',
            meeting: 'M8 7V3a2 2 0 012-2h6a2 2 0 012 2v4M8 7h8M8 7l4 8M8 15l4-8'
        };
                                                ...
                                                <div class="flex items-center space-x-3">
                                                    <div class="w-8 h-8 bg-[#c174f2] bg-opacity-10 rounded-full flex items-center justify-center">
                                                        <svg class="w-4 h-4 text-[#c174f2]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{getActivityIcon(activity.icon || activity.type)}"></path>
                                                        </svg>
                                                    </div>
                                                    <div class="flex-1">
                                                        <p class="text-sm text-gray-900">{activity.message || activity.description}</p>
                                                        <p class="text-xs text-gray-500">
                                                            {activity.time || (activity.created_at ? new Date(activity.created_at).toLocaleString('fr-FR') : 'Récemment')}
                                                        </p>
                                                    </div>
                                                </div>
                                            {/each}
                                        ...
                                        {#each userProjects as project}
                                            <div class="border border-gray-200 rounded-xl p-6 hover:border-[#c174f2] hover:shadow-lg transition-all duration-300 cursor-pointer"
                                                ...
                                                <div class="flex items-center justify-between mb-4">
                                                    ...
                                                    <span class="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-full">{project.sector || 'N/A'}</span>
                                                </div>
                                                <h4 class="font-bold text-gray-900 mb-2">{project.name || 'Nom non disponible'}</h4>
                                                <p class="text-gray-600 text-sm mb-4 line-clamp-3">{project.description || 'Description non disponible'}</p>
                                                <div class="flex items-center justify-between text-sm">
                                                    <span class="text-[#c174f2] font-medium">{project.maturity || 'N/A'}</span>
